...
function MalleoAjaxLoad(texte){
	...
	httpRequestM.open( 'POST' , 'fonctions/includes/inc_tinyMCE_apercu.php' , true );
	httpRequestM.onreadystatechange = function()
										{
											if( httpRequestM.readyState == 4 )
											{
												mon_texte = httpRequestM.responseText;
											}
										}
	httpRequestM.setRequestHeader( "Content-type" , "application/x-www-form-urlencoded" );
	httpRequestM.send( 'texte='+escape(texte));
}

function MalleoTempo(){
	if (mon_texte==''){
		setTimeout("MalleoTempo()", 50);
	}else{
		win = window.open("", "mcePreview", "menubar=no,toolbar=no,scrollbars=yes,resizable=yes,left=20,top=20,width=920,height="  + 600);
		win.document.write(mon_texte);
		win.document.close();
		return true;
	}
}
