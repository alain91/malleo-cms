...
{
	global $droits,$session,$_SESSION,$lang,$_POST;
...
if(isset($_POST['mail_mdp'])){
	...
	if ($_POST['login']=='' || !chk_crypt($_POST['code'])){
		// on invite le user a recommencer
		$essais++;
		if ($essais > 5) hacking_detection($session->time+3600);
...
}elseif (isset($_POST['authentifier']))
{
	...
	{	
		...
		$essais = (isset($_SESSION['essais_login']))?intval($_SESSION['essais_login']):0;
		$essais++;
		if ($essais >= 3) hacking_detection($session->time+3600);
		$_SESSION['essais_login'] = $essais;
