...
$sql = 'SELECT u.*, (YEAR(CURRENT_DATE)-YEAR(u.date_naissance))- (RIGHT(CURRENT_DATE,5)<RIGHT(u.date_naissance,5)) AS age, s.date_lastvisite 
...
if(isset($row['pseudo']))
{
	...
	$tpl->assign_block_vars('found', array());
	...
	if ($row['yahoo']!='')	$tpl->assign_block_vars('found.yahoo', array());
	if ($row['msn']!='')	$tpl->assign_block_vars('found.msn', array());
	if ($row['icq']!='')	$tpl->assign_block_vars('found.icq', array());
	if ($row['gtalk']!='')	$tpl->assign_block_vars('found.gtalk', array());
	if ($row['site_web']!='')	$tpl->assign_block_vars('found.site_web', array());


	if ($user['user_id']==$user_id || $user['level']>=9) $tpl->assign_block_vars('found.droit_edition', array());
	...
	{
		$tpl->assign_block_vars('found.categories_profil', array(
		...
		if ($user['user_id']==$user_id || $user['level']>=9) $tpl->assign_block_vars('found.categories_profil.droit_edition', array());
	}
}
else
{
	$tpl->assign_block_vars('not_found', array());
}
?>
