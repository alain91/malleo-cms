...
class xPDO {
                ...
                if (XPDO_PHP4_MODE || !$this->_nativeMode) {
                    ...
                    if (is_object ($this->pdo)) {
						...
						if (!$connected) {
							...
							$msg = empty($info[2]) ? 'no message' : $info[2];
							$this->pdo = null;
							throw new PDOException('PHP4 or Emulated : '.$msg);
						}
                    }
                } else {
     ...
     */
    function setPackage($pkg= '', $path= '', $prefix= null) {
        $set= false;
     ...
     * @param string|null $prefix Provide a string to define a package-specific table_prefix.
     */
    function addPackage($pkg= '', $path= '', $prefix= null) {
        $added= false;
