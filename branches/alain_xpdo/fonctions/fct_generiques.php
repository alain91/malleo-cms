...
function error404($msg=false)
{
     ...
     *
     * _Id: Kohana.php 1899 2008-02-02 00:29:35Z Shadowhand _ //remplacement des $ par _
     *
     * @package    Core
     * @author     Kohana Team
    ...
    * @return  string
    *
    * adaptation a malleo par alain91
    */
    function malleo_ko_backtrace($trace)
    {
	 ...
	 * @param   array           variables context
	 * @return  void
     *
     * adaptation a malleo par alain91
	 */
	function malleo_ko_error_handler($exception, $message = NULL, $file = NULL, $line = NULL, $context = NULL)
	{
		...
		if ($PHP_ERROR)
		{
			$code     = $exception;
			$type     = 'PHP Error';
            // les autres params sont dans les arguments d'appel de la fonction
		}
		else
		{
		...
		{
			// Log the error
			// Log::add('error', 'uncaught_exception', $type, $message, $file, $line);
		}
		...
		{
            // appel fonction affichage erreur minimale
		}
...
function malleo_display_error($code=E_USER_ERROR, $message=1, $file='', $line='', $context='')
{
    ...
    if (is_numeric($message)){
        $num=intval($message);
        if ($num>=0) {
            $error_msg = !empty($erreur[$num])?$erreur[$num]:$num;
        }else{
            ...
            $sql_geshi = $geshi->parse_code();
            $num2=-1*$num;
            $error_msg = !empty($erreur[$num2])?$erreur[$num2]:$num2;
        }
    }else{
        $error_msg=$message;
    }
	...
	$tpl->assign_vars(array(
		...
		'ERROR_LINE'	=> $line,
		'ERROR_FILE'	=> $file,
		'ERROR_CODE'	=> !empty($code_erreur[$code])?$code_erreur[$code]:'Unknown error level ('.$code.')',
		'ERROR_MSG'		=> $error_msg,
	));
